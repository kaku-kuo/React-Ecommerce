// About products
export const GET_PRODUCTS = "GET_PRODUCTS";
export const GET_PRODUCT = "GET_PRODUCT";
export const GET_ALL_PRODUCTS = "GET_ALL_PRODUCTS";
export const GET_TOP_PRODUCTS_SUCCESS = "GET_TOP_PRODUCTS_SUCCESS";
export const GET_TOP_PRODUCTS_FAIL = "GET_TOP_PRODUCTS_FAIL";
export const ADD_REVIEW_SUCCESS = "ADD_REVIEW_SUCCESS";
export const ADD_REVIEW_FAIL = "ADD_REVIEW_FAIL";

// About user
export const REGISTER_SUCCESS = "REGISTER_SUCCESS";
export const REGISTER_FAIL = "REGISTER_FAIL";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAIL = "LOGIN_FAIL";
export const USER_LOGOUT = "USER_LOGOUT";
export const USER_LOADED = "USER_LOADED";
export const USER_UPDATED = "USER_UPDATED";
export const UPDATE_FAIL = "UPDATE_FAIL";

// About shopping cart, checkout process
export const ADD_TO_CART = "ADD_TO_CART";
export const REMOVE_ITEM_FROM_CART = "REMOVE_ITEM_FROM_CART";
export const CLEAN_CART_ITEMS = "CLEAN_CART_ITEMS";
export const EDIT_ITEM_FROM_CART = "EDIT_ITEM_FROM_CART";
export const SAVE_SHIPPING_ADDRESS = "SAVE_SHIPPING_ADDRESS";
export const SAVE_PAYMENT_METHOD = "SAVE_PAYMENT_METHOD";
export const CREATE_NEW_ORDER_REQUEST = "CREATE_NEW_ORDER_REQUEST";
export const CREATE_NEW_ORDER_SUCCESS = "CREATE_NEW_ORDER_SUCCESS";
export const CREATE_NEW_ORDER_FAIL = "CREATE_NEW_ORDER_FAIL";

// About admin
export const CREATE_NEW_PRODUCT_SUCCESS = "CREATE_NEW_PRODUCT_SUCCESS";
export const CREATE_NEW_PRODUCT_FAIL = "CREATE_NEW_PRODUCT_FAIL";
export const UPDATE_PRODUCT_SUCCESS = "EDIT_PRODUCT_SUCCESS";
export const UPDATE_PRODUCT_FAIL = "EDIT_PRODUCT_FAIL";
export const REMOVE_PRODUCT = "REMOVE_PRODUCT";
export const CLEAN_NEW_CREATE_PRODUCT = "CLEAN_NEW_CREATE_PRODUCT";

// About orders
export const GET_ORDER = "GET_ORDER";
export const GET_ORDERS = "GET_ORDERS";
export const UPDATE_ORDER_SUCCESS = "UPDATE_ORDER_SUCCESS";
export const UPDATE_ORDER_FAIL = "UPDATE_ORDER_FAIL";
export const ORDER_ERROR = "ORDER_ERROR";

// Others
export const PRODUCTS_ERROR = "PRODUCTS_ERROR";
export const SET_LOADING = "SET_LOADING";
export const SET_ALERT = "SET_ALERT";
export const REMOVE_ALERT = "REMOVE_ALERT";
export const AUTH_ERROR = "AUTH_ERROR";
export const CLEAR_ERROR = "CLEAR_ERROR";
export const CLEAR_REVIEW_ERROR = "CLEAR_ERROR";

